...
def plot_w_marginal(plot_dir, results_dir_marg, results_dir_cond, x, methods2plot, alpha, beta=0.4, **kwargs):
    subset_exp = 'SPPI [subset]' in methods2plot
...
def get_args():
    ...
    parser.add_argument('--subset_exp', action='store_true', help='Plot experiment with SPPI [subset]. Note: you must pass k={} when using this flag.')
...
if __name__ == "__main__":
    ...
    if args.subset_exp:
        methods2plot = ['SC', 'SPPI', 'SPPI [subset]']
    else:
        methods2plot = ['SC', 'SC (synth)', 'SPPI']
    if args.meps:
